...
if terraform_location_env != False and len(sys.argv) > 1 and sys.argv[1] == '--list':
    ...
    ip_list = terraform_state_json["outputs"]["external_ip_address_docker-host"]["value"]
    hosts = []
    host_vars = {}

    for i in range(len(ip_list)):
        ansible_host = "docker-host-%d" % i
        hosts.append(ansible_host)
        host_vars[ansible_host] = {"ansible_host": ip_list[i]}

    inventory_json = {"dockerhost": {"hosts": hosts}, "_meta": {"hostvars": host_vars}}
